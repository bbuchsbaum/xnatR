...
require_auth <- function() {
  if (is.null(xnatR_env$base_url) || is.null(xnatR_env$username) || is.null(xnatR_env$password)) {
    abort_auth_required()
  }
  invisible(TRUE)
}
...
#' @noRd
xnat_get <- function(path, query = NULL) {
  req <- xnat_request(path)
...
#' @noRd
xnat_get_tibble <- function(path, query = NULL, class_name = NULL) {
  json <- xnat_get(path, query)
...
#' @noRd
xnat_post <- function(path, body = NULL, query = NULL) {
  req <- xnat_request(path)
...
#' @noRd
xnat_download <- function(path, dest_file, query = NULL, progress = TRUE) {
  req <- xnat_request(path)
